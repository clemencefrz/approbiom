// Type definitions for the Grist plugin API (the `grist` global that
// grist-plugin-api.js installs on `window`).
//
// VENDORED — do not edit by hand, and keep it out of Prettier/ESLint so it
// stays diffable against upstream (see .prettierignore, eslint.config.js).
// Source: https://github.com/gristlabs/grist-widget/blob/master/inspect/grist-plugin-api.d.ts
// Upstream last changed 2022-10-27 (commit 36b8283).
//
// To regenerate it, use the inspect/generate.js script from gristlabs/grist-widget.
// That script bundles the .d.ts out of a *built* Grist checkout (~/dev/grist
// with _build/), so it is only worth the trouble if this copy turns out to lag
// the API we need — otherwise just re-copy the file linked above.
// Then, remove the quotes around 'grist' in the first module declaration:
// `declare module 'grist'` declares an importable module, but the widget gets
// `grist` as a global. Here that edit is written as the equivalent, and
// clearer, `declare namespace grist`.
//
